...
public class ServletAdicionarFilme extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		boolean resultado = afd.adicionarFilme(titulo, sinopse, genero, c, avaliacao, duracao, sqlDate);
				
		if(resultado){
			RequestDispatcher rd = request.getRequestDispatcher("/listarFilmes");
			rd.forward(request, response);
		}else{
			request.setAttribute("mensagem", "Erro inesperado,Filme não adicionado.");
